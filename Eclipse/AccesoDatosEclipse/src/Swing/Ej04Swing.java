	...
	public void actionPerformed(ActionEvent e) {
		...
		if ((e.getSource()) == btnCalcular) {
			if (comprobarJTextFieldsNain()) {
				...
				double perimetro = Punto.distancia2Puntos(punto1, punto2) + Punto.distancia2Puntos(punto2, punto3)
						+ Punto.distancia2Puntos(punto3, punto1);
	...
	public void keyTyped(KeyEvent e) {
		try {
			...
			if (c == '-' && ((JTextField) e.getSource()).getText().contains("-")
					&& ((JTextField) e.getSource()).getText().charAt(0) == '-')
				e.consume();
			if (c == '.' && ((JTextField) e.getSource()).getText().contains(".")
					&& ((JTextField) e.getSource()).getText().charAt(0) == '-') {
				e.consume();
